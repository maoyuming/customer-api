...
public class RefundReqData {
     ...
     * @param totalFee 订单总金额，单位为分,此方法退总金额
     * @param refundFeeType 货币类型，符合ISO 4217标准的三位字母代码，默认为CNY（人民币）
     */
    public RefundReqData(String transactionId,String outRefundNo,int totalFee){
    	
        //微信分配的公众号ID（开通公众号之后可以获取到）
        setAppid(PayConfig.WECHAT_APPID);
        
        //微信支付分配的商户号ID（开通公众号的微信支付功能之后可以获取到）
        setMch_id(PayConfig.WECHAT_MCHID);
        
        //transaction_id是微信系统为每一笔支付交易分配的订单号，通过这个订单号可以标识这笔交易，它由支付订单API支付成功时返回的数据里面获取到。
        setTransaction_id(transactionId);

        //微信支付分配的终端设备号，与下单一致
//        setDevice_info(transactionID);
        
        setOut_refund_no(outRefundNo);
        setTotal_fee(totalFee);
        setRefund_fee(totalFee);
        setOp_user_id(PayConfig.WECHAT_APPID);
        //随机字符串，不长于32 位
