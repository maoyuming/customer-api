...
import com.duantuke.sc.face.service.PayService;
...
//            }
     ...
     * 
     * @ResponseBody public ResponseEntity<OpenResponse<Object>>
     * refund(HttpServletRequest request, HttpServletResponse response, Long
     * orderId) {
     * 
     * boolean r = false; log.info("订单退款, orderId:{}", orderId);
     * 
     * String redisKey = null; String redisValue = null;
     * 
     * OpenResponse<Object> openResponse = new OpenResponse<Object>();
     * 
     * try {
     ...
     * 
     * String lockValue = redisCacheManager.tryLock(redisKey, 10);; if
     * (StringUtils.isNotEmpty(lockValue)) { log.info("订单:" + redisKey +
     * "获取分布锁成功,继续执行退款流程.");
     * 
     * redisValue = lockValue; } else { log.info("订单:" + redisKey +
     * "获取分布锁失败,返回.");
     * 
     * openResponse.setResult(Constants.FAIL);
     * openResponse.setErrorCode(ErrorEnum.accntRefunding.getId());
     * openResponse.setErrorMessage(ErrorEnum.accntRefunding.getName()); return
     * new ResponseEntity<OpenResponse<Object>>(openResponse, HttpStatus.OK); }
     * 
     * ScPay pay = settlementService.getPay(orderId);
     * 
     * String thirdNo = pay.getThirdNo(); BigDecimal sum = pay.getPrice();
     * if(pay.getPayChannel() == 1) {
     * 
     * log.info("订单:{}进行支付宝退款.", orderId); r = AliPay.refund(orderId, thirdNo,
     * sum); }
     * 
     * if (pay.getPayChannel() == 2) {
     * 
     * log.info("订单:{}进行微信退款.", orderId); r = WXPay.refund(orderId, thirdNo,
     * sum.divide(sum).intValue()); } log.info("订单:{}退款完毕, 结果:{}", orderId, r);
     * 
     * if (r) { settlementService.refund(pay);
     * openResponse.setResult(Constants.SUCCESS); } else {
     * openResponse.setResult(Constants.FAIL); } openResponse.setData(r);
     * 
     * 
     * } catch (Exception e) { log.error("退款错误", e);
     * 
     * openResponse.setResult(Constants.FAIL);
     ...
     * redisCacheManager.releaseLock(redisKey, redisValue); }
     * 
     * return new ResponseEntity<OpenResponse<Object>>(openResponse,
     * HttpStatus.OK); }
     */
    ...
    public void weChatCallback(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            if ("SUCCESS".equals(map.get("return_code")) && "SUCCESS".equals(map.get("result_code"))) {
                ...
                log.info("微信支付失败, return_code:{}, result_code:{}, err_code:{}, err_code_des:{}", map.get("return_code"),
